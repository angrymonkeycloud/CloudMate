...
/// <summary>Runs <c>mate --all</c> in the project root of the selected item.</summary>
internal sealed class BuildAllVsCommand : VsCommandBase
{
    ...
    public static async Task InitializeAsync(AsyncPackage package)
    {
        ...
        var instance = new BuildAllVsCommand(package);
    ...
    /// <summary>Build All is only offered on the .mateconfig.json file.</summary>
    ...
    private void Execute(object sender, EventArgs e)
    {
        ...
        {
            Log("[CloudMate] Build All: please select a .mateconfig.json inside a project.");
            return;
        }

        Log($"> mate --all  [{workingDir}]");
        RunBuild(workingDir, new[] { "--all" });
    }
}
